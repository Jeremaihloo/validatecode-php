...
function ashu_rewrite_rules( $wp_rewrite ){
	$new_rules = array(
		'my-account/?$' => 'index.php?my_custom_page=contact',
		'contact/?$' => 'index.php?my_custom_page=contact',
		'validate-code/?$' => 'index.php?my_custom_page=op-get-validate-code',
	); //添加翻译规则
...
//	var_dump($wp_query->query_vars);
	if(isset($wp_query->query_vars['my_custom_page'])) {
		$redirect_page = urldecode($wp_query->query_vars['my_custom_page']);

		//如果my_custom_page等于hello_page，则载入user/helloashu.php页面
		//注意 my-account/被翻译成index.php?my_custom_page=hello_page了。
		$path = TEMPLATEPATH."/user/".$redirect_page.".php";
//		echo 'path:'.$path;
		if(file_exists($path)){
			include($path);
			die();
